...
ggplot(data = webs, aes(x = temp.C, y = length)) + geom_point(col = "gray37") + 
  geom_point(x = webs[36, 3], y = webs[36, 2], col = "firebrick1", size = 3) + 
    geom_point(x = webs[36, 3], y = 0.9981612, col = "palegreen3", size = 3) + 
  annotate(geom = "text", x = webs[36, 3] + 1.5, y = 0.9981612, label = "hat(Y[i])", parse = TRUE, col = "palegreen3", size = 5) +
  annotate(geom = "text", x = webs[36, 3] + 1, y = webs[36, 2] - 0.0001, label = "Y[i]", parse = TRUE, col = "firebrick1", size = 5) +
  ...
  labs(x = "Temperature (in C)", y = "Relative spider web length") + theme_classic() + theme(text = element_text(size = text.size))
```
